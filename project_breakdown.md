...
│   │   ├── stt.js              # ElevenLabs STT wrapper (formerly whisper)
...
- **`backend/data/words.json`**: Building out the robust, themed dictionary of words with image paths and initial sentences.
- **Conversation Scripts**: Writing the JSON structures for the real-world dialogues (e.g., the worker vs. user interaction in a grocery store).
- **Testing & QA**: Acting as the primary tester of the app to ensure the "Hear → Repeat" loop is unbroken and feels completely intuitive without text.
- **Demo Script**: Writing the 3-minute presentation. You will likely be the one speaking and demoing the app to the hackathon judges.
